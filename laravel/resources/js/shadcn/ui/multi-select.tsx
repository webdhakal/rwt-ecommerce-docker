...
import { CheckIcon, ChevronDown, Eye, EyeClosed, WandSparkles } from 'lucide-react';
...
interface MultiSelectProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof multiSelectVariants> {
    ...
    /**
     * Placeholder icon to be displayed when no values are selected.
     * Optional, defaults to "CirclePlus".
     */
    // placeholderIcon?: any
     ...
     */
    className?: string;
    noItemSelectedPlaceholder?: string;
    showBadge?: boolean;
}
    ...
    (
        {
            ...
            noItemSelectedPlaceholder = 'Show Columns',
            animation = 0,
            maxCount = 2,
            modalPopover = false,
            asChild = false,
                ...
                <PopoverTrigger asChild>
                                ...
                                <Label className="px-2 text-black">
                                    {selectedValues.length > 0 ? (
                                        ...
                                        <span className="flex items-center gap-2">
                                            {showBadge && <Eye />} {noItemSelectedPlaceholder}
                                        </span>
                                    )}
                                </Label>
                  ...
                  {selectedValues.length > maxCount && (
                    <Badge
                      ...
                      style={{ animationDuration: `${animation}s` }}
                    >
                      {`+ ${selectedValues.length - maxCount} ${showBadge ? 'more' : 'item'}`}
                      <XCircle
                        className="ml-2 h-4 w-4 cursor-pointer"
                        onClick={(event) => {
                          event.stopPropagation()
                  ...
                  /> */}
                            ...
                            <div className="mx-auto flex w-full items-center justify-between">
                                <p className="mx-3 flex items-center gap-1 text-sm text-black">
                                    {showBadge && <EyeClosed />} <span className="hidden lg:block">{placeholder}</span>
                                </p>
                                <ChevronDown className="mx-1 hidden h-4 cursor-pointer text-muted-foreground lg:block" />
                            </div>
                        )}
                    </Button>
                </PopoverTrigger>
