...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
    ({ children, inputParentClassName, className, error, icon, type, renderAfterComponent, ...props }, ref) => {
        ...
        return (
            <>
                <div
                    className={cn('relative flex w-full items-center rounded-md border border-input bg-transparent shadow-sm', inputParentClassName)}
                >
        ...
        return (
            <div className={cn('space-y-1', className)}>
                {label && (
                    <label
                        htmlFor={props.id}
                        className={cn('text-sm font-medium text-muted-foreground', required && 'required-field', labelClassName)}
                    >
                        {label}
                    </label>
                )}
                ...
                <div className="relative w-full">
                    ...
                    {clipboardCopy && typeof value === 'string' && (
                        <button
                            type="button"
                            onClick={handleCopy}
                            className="absolute top-1/2 right-2 -translate-y-1/2 rounded-md p-1 transition-colors hover:bg-accent"
                            title="Copy to clipboard"
                        >
                            {copied ? <Check size={16} className="text-muted-foreground" /> : <Copy size={16} className="text-muted-foreground" />}
                        </button>
                    )}
                </div>
...
const TextAreaWithLabel = React.forwardRef<HTMLTextAreaElement, TextAreaWithLabelProps>(
    ({ children, label, subLabel, textAreaClassName, error, labelClassName, className, placeholder, required, ...props }, ref) => {
        return (
            <div className={cn('space-y-1', className)}>
                {label && (
                    <label
                ...
                <Textarea ref={ref} placeholder={placeholder} className={cn(textAreaClassName)} {...props} />
                {subLabel && <label className={cn('text-xs font-medium text-muted-foreground', labelClassName)}>{subLabel}</label>}
                {error && <InputError message={error} />}
            </div>
        );
    },
...
const InputWithColor = React.forwardRef<HTMLInputElement, InputWithColorProps>(
    ({ label, subLabel, color, error, labelClassName, colorClassName, inputClassName, ...props }, ref) => {
        return (
            <div>
                {label && (
                    <Label htmlFor={props.id} className={cn('text-sm font-medium text-muted-foreground', labelClassName)}>
                        {label}
                    </Label>
                )}
                <Input
                    ref={ref}
                    className={cn('text-white select-none focus-visible:ring-0', inputClassName)}
                    {...props}
                    style={{
...
const InputWithTags = ({ label, tags, placeholder, disable, setTags, ...props }: InputWithTagsProps) => {
...
export { Input, InputWithColor, InputWithLabel, InputWithTags, TextAreaWithLabel };
