...
const SwitchWithLabel: React.FC<SwitchWithLabelProps> = ({ label, labelclassName, checked, onCheckedChange, error }) => {
    return (
        <div>
            <div className={cn('flex items-center gap-2 p-2')}>
                <Switch checked={checked} onCheckedChange={onCheckedChange} />
                <Label className={cn('text-sm font-medium text-muted-foreground', labelclassName)}>{label}</Label>
            </div>
