        ...
        {relatedProducts?.map((product) => (
          ...
          >
            <div className="relative">
              ...
              {/* Badge */}
              {product?.badge && (
                <div className={`absolute top-2 left-2 px-2 py-1 rounded-md text-xs font-medium ${product?.badge === 'Best Seller' ? 'bg-warning text-white' :
                  product?.badge === 'New' ? 'bg-success text-white' :
                    product?.badge === 'Popular' ? 'bg-accent text-white' : 'bg-muted text-text-secondary'
                  }`}>
                  {product?.badge}
                </div>
              )}
