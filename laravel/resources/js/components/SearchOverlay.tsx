...
import { Command, CommandGroup, CommandList } from '@/shadcn/ui/command';
...
const SearchOverlay: React.FC<SearchOverlayProps> = ({
    ...
    return createPortal(
        <div ref={overlayRef} className="fixed inset-0 z-[9999]">
            ...
            {/* Search Container */}
            <div className="fixed inset-0 z-[10000] flex items-start justify-center px-4 pt-32" style={{ pointerEvents: 'none' }}>
                <div className="w-full max-w-2xl" style={{ pointerEvents: 'auto' }} onClick={(e) => e.stopPropagation()}>
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full rounded-full border border-border bg-background py-4 pr-6 pl-12 text-foreground shadow-lg ring-2 ring-ring/20 outline-none focus:ring-2 focus:ring-primary"
                            value={query}
                    ...
                    <Command className="mt-4 max-h-[60vh] overflow-y-auto rounded-xl border bg-background p-4 shadow-lg">
                        <CommandList>
                                    ...
                                    <div className="mt-2 flex space-x-3 overflow-x-auto pb-2" onWheel={onWheel}>
                                        {categories.map((cat, index) => (
                                            <div
                                                ...
                                                onClick={() => onClose()}
                                            >
                                                <img className="h-16 w-16 rounded-lg bg-card object-cover" src={randomImage(index)} alt={cat.name} />
                            ...
                            {isLoading ? (
                                ...
                                <div className="p-4 text-center text-red-500">Failed to load search results</div>
                                    ...
                                    <div className="space-y-2">
                                        {searchResults.map((product: any) => (
                                            <Link href={`/product/${product?.slug}`} key={product.id}>
                                                <div
                                                    className="flex cursor-pointer items-center space-x-4 rounded-lg p-3 transition-colors hover:bg-accent/50"
                                                    onClick={() => {
                                                        onClose();
                                                    }}
                                                >
