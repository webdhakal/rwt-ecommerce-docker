...
export function RandomPromoCard({
    ...
    useEffect(() => {
        if (randomAppear) {
            const randomTime = Math.floor(Math.random() * (randomInterval.max - randomInterval.min + 1)) + randomInterval.min;
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            {open && <DialogOverlay className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm transition-opacity" />}
            <DialogContent
                className={cn(
                    'fixed top-1/2 left-1/2 z-[9999] w-[95%] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg bg-white shadow-lg transition-all duration-300 ease-in-out',
            ...
            >
                <div className="flex flex-col lg:flex-row">
                    <div className="space-y-4 p-6 sm:p-8 md:p-10 lg:w-2/3">
                        <h1 className="text-xl font-bold text-gray-800 sm:text-2xl md:text-3xl lg:text-4xl">{title}</h1>
                        ...
                        <Button
                            ...
                            className="mt-4 bg-primary px-6 py-3 text-white hover:bg-primary/90"
                        >
                            {buttonLabel}
                        </Button>
                    </div>
                    <div className="hidden items-center justify-center bg-primary p-6 sm:p-8 md:flex md:p-10 lg:w-1/3">
                        <img src="https://placehold.co/400x300?text=Picture+Here" alt="App Screenshot" className="h-auto max-w-full rounded-lg" />
                    </div>
                </div>
            </DialogContent>
        </Dialog>
