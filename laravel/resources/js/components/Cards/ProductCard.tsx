            ...
            <DialogContent className="mx-auto w-full max-w-3xl p-6" onClick={(e) => e.preventDefault()}>
                <div className="mt-4 flex flex-col gap-6 sm:flex-row">
                    <div className="sm:w-1/2">
                        <ZoomableImage src={product.imageUrl} alt={product.name} className="flex-center" zoomScale={2.5} />
                    </div>
                    <div className="flex flex-col justify-between sm:w-1/2">
                        <div>
                            ...
                            <p className="font-secondary mt-4 line-clamp-4 text-sm text-gray-600 dark:text-gray-300">{product.description}</p>
                        </div>
                        ...
                        <div className="mt-4">
                            <div className="flex items-center space-x-4">
                                <span className="text-xl font-bold text-gray-800 dark:text-white">
                                    Rs. {product.price}
                                    {product.discount && (
                                        <span className="ml-2 text-base text-gray-500 line-through dark:text-gray-400">Rs. {product.discount}</span>
                                    )}
                                </span>
                            </div>
                            <div className="mt-2 space-x-2">
                                <Button className="font-secondary">Add to Cart</Button>
                                <Button variant="outline" className="font-secondary border-primary" href="#">
                                    More Details
                                </Button>
                            </div>
                        </div>
...
const ProductCard = ({ product, className, style }: { product: ProductType; className?: string; style?: object }) => {
    ...
    return (
        <Link href="#">
            <div
                className={cn(
                    'group shadow-2 w-full transform cursor-pointer overflow-hidden rounded-md bg-white transition-all duration-200 dark:bg-transparent dark:ring-1 dark:ring-slate-700',
                    className,
                )}
                style={style}
            >
                <div className="relative">
                    ...
                    {product.badge && (
                        <Badge className={cn('absolute top-2 left-2 text-xs font-normal capitalize', productBadgeColor(product.badge.toString()))}>
                            {product.badge}
                        </Badge>
                    )}
                </div>
                <div className="space-y-1 p-3 lg:px-4 lg:py-3">
                    <div className="flex flex-col items-start">
                        <span className="font-secondary mt-1 block text-xs text-slate-400">{product.category}</span>
                        <h2 className="text-card-title line-clamp-1 font-semibold text-gray-800 dark:text-slate-300">{product.name}</h2>
                        <div className="mb-1 flex items-center gap-2">
                            <RatingStart rating={product.rating} />
                        </div>
                    </div>
                    <span className="text-2xsm">
                        By <span className="font-secondary text-green-500">{product.vendorName || 'unknown'}</span>
                    </span>
                    <div className="flex-between">
                        <div className="text-card-title space-x-2">
                            <span className="font-semibold text-gray-900 dark:text-white">Rs.{product.price}</span>
                            {product.discount && (
