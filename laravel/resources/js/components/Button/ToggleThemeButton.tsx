...
const ToggleThemeButton = ({ className }: { className?: string }) => {
    ...
    return (
        ...
        >
            <span
                className={cn(
                    'absolute top-1 left-1 size-4 transform rounded-full bg-white shadow transition-transform duration-200',
            ...
            <span className="absolute inset-0 flex items-center justify-center">
                <Moon
                    className={cn(
                        'absolute top-1/2 right-1 size-4 -translate-y-1/2 text-gray-600 transition-all duration-200',
                        darkMode ? 'translate-x-4 opacity-0' : 'translate-x-0 opacity-100',
                    )}
                />
                <Sun
                    className={cn(
                        'absolute top-1/2 left-1 size-4 -translate-y-1/2 text-yellow-500 transition-all duration-200',
