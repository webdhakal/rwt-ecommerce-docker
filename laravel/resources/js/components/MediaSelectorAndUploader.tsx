...
import { File, HardDriveUpload, Video, X } from 'lucide-react';
...
const SelectableMediaCard = ({ media, isSelected, onClick }: { media: MediaType; isSelected: boolean; onClick?: () => void }) => (
    <img
        src={media.url}
...
const MediaSelectorAndUploader = ({ setData, images = [], videos = [], files = [], uploadedMedias = [] }: MediaSelectorAndUploaderProps) => {
    ...
    return (
        <Dialog open={isDialogOpen} onOpenChange={() => setIsDialogOpen(!isDialogOpen)}>
            ...
            <DialogTrigger asChild>
                <Button
                    variant="outline"
                    className="flex w-full items-center gap-2 rounded-lg border-2 border-dashed border-muted-foreground px-4 py-2 text-sm hover:bg-muted/40"
            ...
            <div className="grid grid-cols-3 gap-2">
                {uploadedMedias.length > 0 ? (
                    ...
                    <p className="col-span-full text-center text-muted-foreground">No media has been uploaded.</p>
                )}
            </div>
            <DialogContent className="sm:max-w-2xl">
                ...
                <Tabs defaultValue="url" className="mt-4 w-full">
                    ...
                    <TabsContent value="url" className="mt-4 space-y-4">
                        <Input placeholder="Paste image URL" value={mediaURL} onChange={(e) => setMediaURL(e.target.value)} />
                        ...
                        <Tabs defaultValue="images">
                            <TabsList className="mb-2 grid w-full grid-cols-3">
                                <TabsTrigger value="images">
                                    <img className="mr-1 h-4 w-4" /> Images
                                </TabsTrigger>
                    ...
                    <TabsContent value="upload">
                        <div className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-300 p-6 text-center transition hover:border-blue-400">
                            ...
                            <input id="file-upload" multiple type="file" className="hidden" onChange={handleFileUpload} />
                        </div>
                                    ...
                                    {uploadedFiles.map((uploads, index) => (
                                        <li key={index} className="grid grid-cols-2">
                                            ...
                                            <span onClick={() => handleFileRemoval(index)}>
                                                <X
                                                    size={24}
                                                    className="hover:bg-red ml-2 rounded-md border p-[3px] hover:border-transparent hover:text-white"
                                                />
                                            </span>
                                        </li>
                                    ))}
