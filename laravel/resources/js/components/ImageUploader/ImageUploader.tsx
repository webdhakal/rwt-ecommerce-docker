...
const ImageUploader = <T,>({ className, buttonName, data, setData, allowMultiple = false, uploadProgress, error }: ImageUploaderProps<T>) => {
    ...
    return (
        <div className={cn('space-y-4', className)}>
            <div className="mt-2">{uploadProgress !== undefined && <Progress value={uploadProgress * 100} />}</div>
            <Dialog open={openDialog} onOpenChange={setOpenDialog}>
                    ...
                    <DialogTrigger asChild>
                        {buttonName ? (
                            ...
                            <div className="cursor-pointer rounded-md border-2 border-dashed border-gray-300 p-6 text-center transition hover:bg-gray-50">
                                ...
                                <p className="mt-2 text-sm text-gray-500">Dimension of the logo image should be 600 x 600px</p>
                            </div>
                        )}
                    </DialogTrigger>
                )}
                            ...
                            <Dialog key={index}>
                                <DialogTrigger asChild>
                                    <div className="relative cursor-pointer overflow-hidden rounded-md border">
                                        <img src={url} alt={`Preview ${index}`} className="size-24 w-full object-cover" />
                                        <Button
                                            type="button"
                                            size="icon"
                                            className="absolute top-2 right-2 z-10 size-fit rounded-full bg-white p-1 text-red-500 shadow"
                                            variant="ghost"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                ...
                                <DialogContent className="max-w-4xl p-4">
                                    <img src={url} alt={`Full preview ${index}`} className="h-auto w-full rounded-md" />
                                </DialogContent>
                            </Dialog>
                        ))}
                    </div>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Choose one or multiple images from your computer</DialogDescription>
                    </DialogHeader>
