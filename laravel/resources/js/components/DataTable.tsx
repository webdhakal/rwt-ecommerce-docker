...
const SearchInput: FC<SearchInputProps> = ({ placeholder, search, setParams, params, setTimeDebounce }) => (
    <Input
...
export default function DataTable<T extends WithID>({ data = [], options }: DataTableProps<T>) {
    ...
    const handleSelectionChange = (item: T) => {
        setSelectedEntriesByPage((prev) => {
            ...
            return {
                ...prev,
                [currentPage]: isSelected ? currentSelections.filter((entry) => entry.id !== item.id) : [...currentSelections, item],
            };
        });
    };
    ...
    const [visibleColumns, setVisibleColumns] = useState<string[]>(options.columns.map((col) => col.value));
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                                ...
                                {options.bulkActions.map((action, index) => (
                                    <DropdownMenuItem key={`bulkAction-${action.label}-${index}`} onSelect={() => handleBulkAction(action.action)}>
                                        {action.label}
                                    </DropdownMenuItem>
                                ))}
                            </DropdownMenuContent>
            ...
            <Table>
                <TableHeader>
                    <TableRow className="border-t">
                        ...
                        {visibleColumnsConfig.map((column, index) => (
                            <TableHead key={`visibleColumns-${column.value}-${index}`} className={cn('px-0', column.className)}>
                        ...
                        {(options.actions?.edit || options.actions?.destroy) && <TableHead className="px-0">Actions</TableHead>}
                    </TableRow>
                </TableHeader>
                {(options.actions?.edit || options.actions?.destroy) && (
                    <TableBody>
                        {data.length > 0 ? (
                            data.map((item, index) => (
                                <TableRow key={item.id} className={cn(index % 2 !== 0 && 'bg-slate-100 dark:bg-slate-900')}>
                                    ...
                                    {visibleColumnsConfig.map((column, index) => (
                                        <TableCell
                                            ...
                                            className={cn(
                                                'max-w-[350px] truncate px-0 py-2 text-left align-middle whitespace-nowrap',
                                                column.className,
                                            )}
                                        >
                                            {column.body ? column.body(item) : (item[column.value as keyof T] as ReactNode)}
                                        </TableCell>
                                    ))}
                                    {options.actions && (
