...
const BannerSlider = () => {
    ...
    return (
        <AnimatedSection
            className={cn('bg-ashGray relative my-4 h-fit w-full py-5 dark:bg-slate-900')}
            // style={{ animationDelay: '500ms' }}
            // only add the above style if this is made the second component in the home page (i.e. after the image banner)
        >
            <div className="relative container w-full overflow-hidden" onMouseEnter={stopAutoPlay} onMouseLeave={startAutoPlay} ref={callbackRef}>
                            ...
                            <div className="embla__slide mx-2 flex-[0_0_100%]" key={index}>
                                <div className="relative grid h-full grid-cols-1 place-items-center gap-2 py-6 md:grid-cols-[450px_auto] md:justify-between md:justify-items-start">
                                    <div className="order-2 flex-col space-y-2 text-center md:order-1 md:text-left">
                                            ...
                                            <Typewriter
                                                ...
                                                onInit={(typewriter) => {
                                                    const formattedText = item.title
                                                        .split(' ')
                                                        .map((word, i) => (i === 1 ? `<span class="text-primary">${word}</span>` : word))
                                                        .join(' ');
                                        ...
                                        <p className="font-secondary py-4 text-slate-500">
                                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Inventore modi facilis, provident est sint culpa
                                            excepturi maxime natus voluptate eius.
                                        </p>
                                    ...
                                    <div className="relative order-1 mx-auto h-full self-stretch">
                                        ...
                                        />
                                        <Badge
                                            className="absolute top-4 right-4 z-9999 text-center text-xs font-normal text-white md:rounded-md md:text-sm"
                                            variant="secondary"
                                        >
                                            {item.subtitle}
                                        </Badge>
                ...
                <div className="absolute bottom-0 container hidden w-full space-x-1 md:absolute md:bottom-0 md:left-0 md:block">
