...
const TableFilter: FC<TableFilterProps> = ({ className, params, setParams, setTimeDebounce, title, filter, options, maxCount }) => {
        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button
                    variant="outline"
                    size="sm"
                    className={cn('w-full border-dashed border-gray-400 text-xs hover:bg-gray-200/50 md:w-auto', className)}
                >
                    <CirclePlusIcon className="mr-1 h-4 w-4" />
                    <span>{title}</span>
                    {selectedValues.length > 0 && <FilterBadges selectedValues={selectedValues} options={options} maxCount={maxCount} />}
                </Button>
            </PopoverTrigger>
