                            ...
                            <Fragment key={index}>
                                <BreadcrumbItem key={index}>
                                    {item.href ? (
                                        ...
                                        <BreadcrumbPage className="text-gray-800">{item.label}</BreadcrumbPage>
                                    )}
                                </BreadcrumbItem>
                        ...
                        <>
                                    ...
                                    <BreadcrumbItem>
                                        {isDesktop ? (
                                            <DropdownMenu open={open} onOpenChange={setOpen}>
                                                <DropdownMenuTrigger
                                                    className="breadcrumbs-center flex gap-1 focus:outline-none"
                                                ...
                                                <DropdownMenuContent align="start">
                                                    {breadcrumbs.slice(1, -2).map((item, index) => (
                                                        <DropdownMenuItem key={index} className="text-gray-800 focus:bg-gray-200/50">
                                                            <Link href={item.href || '#'}>{item.label}</Link>
                                                        </DropdownMenuItem>
                                                    ))}
                                                </DropdownMenuContent>
                                            ...
                                            <Drawer open={open} onOpenChange={setOpen}>
                                                <DrawerTrigger className="focus:outline-none" aria-label="Toggle Menu">
                                                    <BreadcrumbEllipsis className="h-4 w-4 shrink-0" />
                                                </DrawerTrigger>
                                                <DrawerContent>
                            ...
                            {breadcrumbs.slice(-ITEMS_TO_DISPLAY + 1).map((item, index) => (
                                <BreadcrumbItem key={index}>
                                    {item.href ? (
                                        <>
                                            <BreadcrumbLink asChild className="max-w-20 truncate text-gray-400 hover:text-gray-800 md:max-w-none">
                                                <Link href={item.href}>{item.label}</Link>
                                            </BreadcrumbLink>
                                            <BreadcrumbSeparator />
                                        </>
                                    ) : (
                                        <BreadcrumbPage className="max-w-20 truncate font-medium text-gray-800 md:max-w-none">
                                            {item.label}
                                        </BreadcrumbPage>
                                    )}
                                </BreadcrumbItem>
