...
const CustomPopOverMenuForFilters = ({ filterList, label }: { filterList: any; label: string }) => {
    const [selectedColumns, setSelectedColumns] = useState<string[]>(filterList.map((item: { label: string; value: string }) => item.value));
    return (
        <div>
            <MultiSelect
                options={filterList}
    ...
    return (
        <div className="w-full space-y-2 border p-4">
            ...
            <div>
                ...
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild className="block">
                        <Button variant="outline" role="combobox" aria-expanded={open} className="w-full text-primary">
            ...
            <div>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger className="block w-full">
                        <Button variant="ghost" className="shadow-1 w-full border">
                            Select Type
                        </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent>
