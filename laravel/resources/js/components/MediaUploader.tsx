...
import { ExternalLink, File, FileText, Save, Search, Trash2, Upload, Video, X } from 'lucide-react';
...
const MediaCard = ({ media, onClick }: { media: MediaType; onClick: () => void }) => (
    <div className="group relative aspect-square cursor-pointer overflow-hidden rounded-md border border-gray-200" onClick={onClick}>
        {media.type?.startsWith('image/') ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-black">
                ...
                <a
                    ...
                    className="bg-opacity-70 absolute top-2 right-2 rounded-full bg-gray-800 p-2 text-white opacity-0 transition-opacity group-hover:opacity-100"
        ...
        <div className="bg-opacity-0 group-hover:bg-opacity-40 absolute inset-0 flex items-end justify-start bg-black p-2 transition-opacity">
            <span className="text-sm font-medium text-white opacity-0 transition-opacity group-hover:opacity-100">{media.name}</span>
        </div>
    </div>
);
        ...
        <Dialog open={!!mediaDetails} onOpenChange={(open) => !open && setMediaDetails(null)}>
            <DialogContent className="sm:max-w-[600px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        View or edit details for this {isImage ? 'image' : isVideo ? 'video' : isPDF ? 'PDF' : 'file'}.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4">
                    ...
                    {isPDF && (
                        <div className="space-y-2">
                            <Label>Preview</Label>
                            <object data={mediaDetails.url} type="application/pdf" className="h-64 w-full rounded-md">
                                <p className="text-sm text-muted-foreground">
                                    PDF preview is not supported in this browser.{' '}
                                    <a href={mediaDetails.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                                        View PDF in new tab
                                    </a>
                                </p>
                            </object>
                    ...
                    {isOtherFile && (
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center justify-center rounded-md bg-gray-100 p-4">
                                ...
                                <a href={mediaDetails.url} target="_blank" rel="noopener noreferrer" className="ml-2 text-blue-500 hover:underline">
...
const AddNewMediaDialog = ({ activeTab }: { activeTab: 'images' | 'videos' | 'files' }) => {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Add New {activeTab === 'images' ? 'Image' : activeTab === 'videos' ? 'Video' : 'File'}</DialogTitle>
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <div className="grid items-center gap-4">
                        <Label htmlFor="name">Name</Label>
                        <Input id="name" className="w-full" placeholder="Media name" value={name} onChange={(e) => setName(e.target.value)} />
                    </div>
                    <div className="grid items-center gap-4">
                        <Label htmlFor="alt">Alt Text</Label>
                        <Input id="alt" className="w-full" placeholder="Media alt text" value={alt} onChange={(e) => setAlt(e.target.value)} />
                    </div>
                    <div className="grid items-center gap-4">
                        ...
                        >
                            ...
                            {isDragActive ? (
                                ...
                                <p className="text-sm text-primary">Drag 'n' drop some {activeTab} here, or click to select files</p>
                            )}
                        </div>
                    </div>
                            ...
                            <ScrollArea className="h-[200px] w-full rounded-md border p-4">
                                <ul className="space-y-2">
                                    {selectedFiles.map((file, index) => {
                                        ...
                                        return (
                                            <li key={index} className="flex items-center justify-between rounded-md bg-secondary p-2">
                ...
                <DialogFooter>
                    <Button variant="outline" onClick={handleClearForm} className="mr-2" disabled={isUploading}>
                        Clear Form
                    </Button>
                    <Button
                        type="submit"
                        ...
                        className={cn(
                            selectedFiles.length > 0 && !isUploading && validationErrors.length === 0 ? 'bg-green-500' : 'bg-primary',
        ...
        <div className="container mx-auto my-12 px-4">
            <Card className="mb-8 shadow-lg">
                ...
                <CardContent>
                    ...
                    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'images' | 'videos' | 'files')} className="mt-4">
                        <TabsList className="grid w-full grid-cols-3">
                            <TabsTrigger value="images">
                                <img className="mr-2 h-4 w-4" /> Images
                            </TabsTrigger>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                {filteredMedia.length > 0 ? (
                    filteredMedia.map((item) => <MediaCard key={item.id} media={item} onClick={() => setMediaDetails(item)} />)
                ) : (
                    <div className="col-span-full flex flex-col items-center justify-center py-12 text-center">
                        <div className="mb-4 rounded-full bg-gray-100 p-4">
                            {activeTab === 'images' ? (
                                <img className="h-8 w-8 text-gray-400" />
                            ) : activeTab === 'videos' ? (
                        ...
                        <p className="mt-2 text-muted-foreground">Try uploading new {activeTab} or adjusting your search</p>
                    </div>
                )}
            </div>
