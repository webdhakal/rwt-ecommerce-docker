...
const BentoSection = () => {
    ...
    return (
        <AnimatedSection className={cn('container my-4 space-y-8')}>
            <DefaultSectionTitle title="Unmissable Offers" subtitle="Don't miss out on these exclusive deals" showButton={false} />
                ...
                {products.map((product, index) => (
                    ...
                    >
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-4 text-white transition-transform duration-300 group-hover:translate-y-0">
                            {product.discount && (
                                <Badge className="mb-2 bg-primary text-sm font-bold tracking-wide text-white">{product.discount}</Badge>
                            )}

                            <h3 className="text-lg font-bold tracking-wide md:text-xl lg:text-2xl">{product.title}</h3>
