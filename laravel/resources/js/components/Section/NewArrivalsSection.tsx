...
import { TabFilterProduct } from '@/types/MockData';
    ...
    return (
        <AnimatedSection className={cn('container mx-auto my-4')}>
            <div className="lg:flex-between">
                <DefaultSectionTitle title="New Arrivals" subtitle="Shop online for new arrivals and get free shipping!" showButton={false} />
                        ...
                        <div className="text-white lg:text-slate-500" key={`recent-product-${index}`}>
                            <span
                                className={cn(
                                    ...
                                    tab === activeTab &&
                                        'rounded-md bg-white px-2 py-1 text-primary hover:text-primary lg:p-0 dark:bg-transparent dark:text-white',
                                )}
                                onClick={() => {
                                    setActiveTab(tab);
                                }}
