    ...
    return (
        <div className="font-secondary flex flex-col items-center">
            <motion.div
                ...
                className="flex-center bg-opacity-20 rounded-md border-none border-gray-100 bg-gray-100 bg-clip-padding text-xl text-white backdrop-blur-md backdrop-filter md:size-16 md:text-3xl lg:text-4xl"
            >
...
const DealsSection = () => {
    ...
    return (
        <AnimatedSection className={cn('container my-4 dark:bg-transparent')} delay={500}>
            <DefaultSectionTitle title="Weekly Deals" subtitle="Grab it before its too late!" buttonContent="All weekly Deals" />
            <Timer endTime={new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)} />
            <div className="responsive-product-grid">
                {weeklyDeals?.map((product, index) => (
                    <ProductCard key={index} product={product} style={{ animationDelay: `${index * 400}ms` }} />
                ))}
            </div>
        </AnimatedSection>
    );
