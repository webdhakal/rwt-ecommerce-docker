...
const PageBannerSection = ({ pageTitle, renderChildren, firstPage }: PageBannerSectionProps) => {
    ...
    return (
        <section className={cn('mt-4', isVisible && firstPage && 'animate-bannerFadeIn')} ref={callbackRef}>
            <div className="item-center mb-6 hidden h-[220px] justify-between rounded-xl bg-green-200 px-8 py-6 shadow-md md:flex lg:px-16 lg:py-8 dark:bg-slate-900">
                <div className="flex-between w-full">
                    <div>
                        <h1 className="text-4xl font-bold text-green-900 capitalize">{formattedPageTitle}</h1>
                        <Breadcrumb className="mt-2">
                            <BreadcrumbList className="flex items-center text-sm text-green-700">
                                ...
                                <BreadcrumbItem>
                                    <BreadcrumbPage className="font-secondary text-green-800 capitalize">{formattedPageTitle}</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
            ...
            <div className="mt-4 md:hidden">
                <h1 className="text-center text-3xl font-extrabold text-green-900 capitalize">{formattedPageTitle}</h1>
                <div className="mt-3 flex justify-center">
                    <Breadcrumb className="flex list-none flex-wrap justify-center space-x-2 text-sm text-green-700">
                        ...
                        <BreadcrumbSeparator className="text-green-600">/</BreadcrumbSeparator>
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-green-800 capitalize">{formattedPageTitle}</BreadcrumbPage>
                        </BreadcrumbItem>
                    </Breadcrumb>
                </div>
            </div>
